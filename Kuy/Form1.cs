...
-5.59E-01
            ...
            for (int i = 0; i < channel1_displacement.Length; i++)
            {
                fftChannel1[i] = new Complex(channel1_displacement[i], 0); //(real part,imaginary part)
                fftChannel2[i] = new Complex(channel2_displacement[i], 0);
                fftChannel3[i] = new Complex(channel3_displacement[i], 0);
            }

            Fourier.Forward(fftChannel1, FourierOptions.NoScaling); //NoScaling:คือไม่ปรับแต่งค่า ใช้ค่าจิงๆเลย
            ...
            Complex[] magnitudeC1 = new Complex[fftChannel1.Length];
            Complex[] magnitudeC2 = new Complex[fftChannel2.Length];
            Complex[] magnitudeC3 = new Complex[fftChannel3.Length];

            for (int i = 0; i < fftChannel1.Length; i++)
            {
                magnitudeC1[i] = fftChannel1[i].Magnitude;
                magnitudeC2[i] = fftChannel2[i].Magnitude;
                magnitudeC3[i] = fftChannel3[i].Magnitude;
            }
            ...
            var model = CreatePlotModel(frequencies, magnitudeC1, magnitudeC2, magnitudeC3);
